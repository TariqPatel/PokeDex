...
final class PokeDetailViewModel: ObservableObject {
    ...
    func getPokeDetails() {
        ...
        guard let url = URL(string: Constants.Strings.pokemonCharacteristicUrl + pokeID) else {return}
        NetworkService<Pokemon>.fetchData(for: url) { (result) in
            switch result {
                case .success(let response):
                    DispatchQueue.main.async {
                        let description =  response.descriptions?.filter( { $0.language.name.contains("en") } )
